...
public class ApacheMultipartParser extends DataParser {
	 ...
	 */
	public static class AutoFileItem implements FileItem<AutoFileItem> {
		...
		public AutoFileItem(FileItemInput stream) {
		 ...
		 *
		 * @param path
		 *            The <code>Path</code> into which the uploaded item should be stored.
		 * @throws Exception
		 *             if an error occurs.
		 */
		@Override
		public AutoFileItem write(Path path) throws IOException {
			if (isInMemory()) {
				FileUtils.writeByteArrayToFile(path.toFile(), get());
			} else {
				...
				if (outputFile != null) {
					 ...
					 */
					if (!outputFile.renameTo(path.toFile())) {
						FileUtils.copyFile(outputFile, path.toFile());
					}
				} else {
					/*
		 ...
		 */
		@Override
		public AutoFileItem setFieldName(String fieldName) {
		 ...
		 */
		@Override
		public AutoFileItem setFormField(boolean state) {
			isFormField = state;
			return this;
		}
		...
		/**
		 * Returns an identifier that is unique within the class loader used to load this class but does not have a
		 ...
		 */
		@Override
		public AutoFileItem setHeaders(FileItemHeaders pHeaders) {
			headers = pHeaders;
			return this;
		}
	}
			...
			while (iter.hasNext()) {
				...
				try {
					try {
                        ...
                        try (InputStream in = item.getInputStream(); OutputStream out = fileItem.getOutputStream()) {
                            in.transferTo(out);
                        }
	...
	/**
	 * Iterator over multipart form items.
	 */
	private class FileItemIteratorImpl implements FileItemInputIterator {
		 ...
		 */
		private class FileItemStreamImpl implements FileItemInput {
				...
				if (maxFileSize != -1) {
                    try {
                        istream = BoundedInputStream.builder()
                            ...
                            .setMaxCount(maxFileSize)
                            .setOnMaxCount(
                                (Long pSizeMax, Long pCount) -> {
		...
		FileItemIteratorImpl(InputStream input, String contentType, String charEncoding) throws FileUploadException, IOException {
			...
			if (maxRequestSize >= 0) {
				...
				input = BoundedInputStream.builder()
                    .setInputStream(input)
                    .setMaxCount(maxRequestSize)
                    .setOnMaxCount((Long pSizeMax, Long pCount) -> {
		...
		/**
		 * Returns, whether another instance of {@link FileItemInput} is available.
		 *
